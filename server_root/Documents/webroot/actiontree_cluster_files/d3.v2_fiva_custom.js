(function() {
  ...
  d3.ns = {
    ...
    qualify: function(name) {
      ...
      return d3_nsPrefix.hasOwnProperty(prefix) ? { 
        ...
        local: (prefix + ":" + name) // fiva customfix here, this should actually makes more sense in general
  ...
  // Custom version of the d3js cluster, for use as the action tree in fiva
  d3.layout.fiva_custom_cluster = function() {
      function fiva_custom_cluster(d, i) {
        ...
        // fiva custom fix
        var shortBranchFixRecursive = function(sBranch) {

            sBranch.y = sBranch.parent.y - 1;

            if(!sBranch.children || !sBranch.children.length){
              return;
            }   

            for(child in sBranch.children) {
              shortBranchFixRecursive(sBranch.children[child]);
            }                            
        }

        var shortBranch; // fiva custom
        d3_layout_treeVisitAfter(root, function(node) {
              node.x = (node.x - x0) / (x1 - x0) * size[0];

              // fiva custom fix, you don't want all branches of the tree to have the same screen length
              // despite having different depth on the tree. Why because visually this is misleading.
              // Sorting the screen length by time may be another way to do this, but for now let's just
              // fix this bug.
              // 
              // the bug is caused by the algorithm for counting tree depth in d3 working from leaf to node
              // so a long and short branches both starts at 0 at the leaf, and yet the node right next to 
              // leaf will be 1 for the long branch and say, 3 for the short branch (say the tree ends with
              // 5 at the root).
              //
              // I did not try to rewrite the algorithm in d3 for fear of breaking something internal (since
              // it's a rather unusual way to count tree depth that I figure there is some special reason
              // to this). So the simple bug fix is to recalibrate all the short branches
              //
              // starting from each leaf node, move upward to the root, see if can find a previous node
              // that is more than one position away from the current
              shortBranch = node;
              while (shortBranch.parent) {
                if ((shortBranch.parent.y - shortBranch.y) > 1)
                {
                  shortBranchFixRecursive(shortBranch);
                } 
                shortBranch = shortBranch.parent;
              }
      ...
      fiva_custom_cluster.separation = function(x) {
          if (!arguments.length) return separation;
          separation = x;
          return fiva_custom_cluster;
        };

      fiva_custom_cluster.size = function(x) {
        if (!arguments.length) return size;
        size = x;
        return fiva_custom_cluster;
      };
    return d3_layout_hierarchyRebind(fiva_custom_cluster, hierarchy);
  };
